# -*- coding: utf-8 -*-
"""
Created on Mon May 31 12:47:55 2021
...
Creating fastText vectors to tranform the data. SVDR and NMFDR are
...
if model_type == 'OM':
    fast_text_dict = fast_text_vectors(words_file)
elif model_type == 'PM':
    fast_text_dict = fasttext_reader(fasttext_file)
elif model_type == 'PM+OM':
    fast_text_dict = fasttext_reader(fasttext_file)
    fast_text_dict_aux = fast_text_vectors(words_file)
...
# Create the corpus with fastText vectors
...
for l in l_s:
    ...
    for w in l:
        if w in fast_text_dict:
            if model_type == 'OM':
                s += fast_text_dict.get_word_vector(w)
            else:
                s += fast_text_dict[w]
            i += 1
        else:
...
ft, labels = remove_nan_values(ft, labels)
ft_corpus = np.array(ft)

# Normalize the data
n, m = ft_corpus.shape
min_values = np.min(ft_corpus, axis=1)
...
for cl in classifiers:
    ...
    for train_index, test_index in skf.split(ft_corpus, labels):
        print('Fold :',i)
        data_train = [ft_corpus[x] for x in train_index]
        data_test = [ft_corpus[x] for x in test_index]
